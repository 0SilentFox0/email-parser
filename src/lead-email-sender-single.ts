...
async function processNewLeads(): Promise<void> {
	try {
		const newLead = await LeadModel.findOne({ emailSent: "pending" });

		// Check if a lead was found
		if (newLead) {
			try {
				await sendPersonalizedEmail(newLead);
			} catch (error) {
				logger.error(`Error processing lead ${newLead._id}:`, error);
			}
		} else {
			logger.info("No new leads with pending email found.");
		}
	} catch (error) {
